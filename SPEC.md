...
- 테스트 설계(Phase 2) 또는 테스트 실행(Phase 3) 중 기획서의 예상 동작과 실제 동작이 다른 경우, **임의로 "설계 의도"로 판단하여 pass 처리하지 않는다**
- 반드시 사용자에게 "기획서에는 X인데 실제로는 Y입니다. 버그인지 설계 변경인지 확인해주세요"라고 질문할 것
- 사용자 확인 전까지는 기획서 기준으로 fail 처리
