...
3. 생성된 `outputs/extract_result.json`, `outputs/scenario_draft_source.md` 참고
4. **참조 이미지**: `outputs/reference/`에 슬라이드/페이지별 이미지 저장됨 → 테스트 시 화면 비교에 사용
5. **구성 체크 리스트**: scenario_draft_source.md 하단 "구성 체크 리스트"에 페이지별 예상 표시 요소 정리됨
6. 테스트 시나리오 Markdown 작성 후 `outputs/scenario_draft.md`에 저장
