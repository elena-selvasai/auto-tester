...
- `scripts/run_all_tests.py` — 테스트 러너 (없으면 `agent-browser`로 직접 재검증)

### agent-browser CLI 사용

DOM 분석 시 `agent-browser` CLI를 사용합니다:

```bash
agent-browser open <URL> && agent-browser wait --load networkidle
agent-browser snapshot -i          # 인터랙티브 요소 + ref 확인
agent-browser get text "<selector>" # 요소 텍스트 조회
agent-browser get attr "<selector>" "<attribute>" # 속성 조회
agent-browser is visible "<selector>" # 가시성 확인
```

`agent-browser`가 설치되지 않은 환경이면 `test_result.json`의 스크린샷·메시지로만 분석합니다.
...
각 실패 테스트에 대해:
1. `agent-browser open <URL> && agent-browser wait --load networkidle`로 테스트 URL 이동
2. 필요한 사전 동작 수행 (`agent-browser click`, `agent-browser fill` 등)
3. `agent-browser snapshot -i`로 실제 DOM 구조 확인
4. `agent-browser get text body`로 전체 텍스트 확인 (필요시)
5. 실패 원인이 된 선택자/텍스트가 실제 DOM에 존재하는지 검증:
   - `agent-browser is visible "<selector>"` — 요소 가시성
   - `agent-browser get text "<selector>"` — 텍스트 내용
   - `agent-browser get attr "<selector>" "<attribute>"` — 속성 값
...
- **실제 DOM**: [실제 발견된 선택자/값]
- **수정 제안**: [새로운 선택자/값]
- **수정 대상 파일**: [scripts/run_all_tests.py / test_plan.json]
- **수정 전**:
  ```python
  # 기존 코드
  ```
...
승인된 수정사항을 적용:
1. `scripts/run_all_tests.py` 수정 (선택자, 기대값, 대기시간 등)
  ...
  "details": [
    {
      ...
      "file_modified": "scripts/run_all_tests.py",
      "before": "원래 값",
      "after": "수정된 값",
      "rerun_status": "passed",
      "github_issue": "https://...",
...
2. 실행 방법:
   - `python scripts/run_all_tests.py --base-url "<URL>" --tc "TC_003,TC_015"` (수정된 TC만 재실행)
   - `python scripts/run_all_tests.py --base-url "<URL>"` (전체 재실행)
   - 또는 `agent-browser`로 수정된 TC만 직접 수동 검증
3. 각 재실행 결과를 기록
...
1. `agent-browser snapshot -i`에서 ref 및 요소 구조 확인
2. `agent-browser eval`로 실제 클래스명 검색:
   ```bash
   agent-browser eval 'JSON.stringify(Array.from(document.querySelectorAll("[class*=\"keyword\"]")).map(e => e.className))'
   ```
